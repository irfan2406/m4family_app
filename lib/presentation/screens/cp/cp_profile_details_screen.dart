...
/// CP profile details — parity with web `app/(cp)/cp/profile/details/page.tsx`.
/// Edit mode for name, email, phone, address, bio + avatar upload (camera button).
class CpProfileDetailsScreen extends ConsumerStatefulWidget {
...
class _CpProfileDetailsScreenState extends ConsumerState<CpProfileDetailsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      debugPrint('CP profile details load: $e');
  ...
  Widget _avatarSection(Color textPrimary) {
    return Center(
      child: Column(
        children: [
          Stack(
