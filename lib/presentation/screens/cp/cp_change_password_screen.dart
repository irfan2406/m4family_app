...
/// CP Change Password — parity with web `app/(cp)/cp/change-password/page.tsx`.
///
/// Mirrors the web prototype: current / new / confirm password fields with a
/// live requirements checklist (8+ chars, uppercase, number, special char).
/// Submits to `PATCH /auth/change-password` via [ApiClient.changePassword].
class CpChangePasswordScreen extends ConsumerStatefulWidget {
  const CpChangePasswordScreen({super.key});

  @override
  ConsumerState<CpChangePasswordScreen> createState() =>
      _CpChangePasswordScreenState();
}
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        // Current password
                        _fieldLabel('CURRENT PASSWORD', muted),
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(14),
          borderSide: BorderSide(color: _purple.withValues(alpha: 0.5)),
        ),
      ),
    );
  }
