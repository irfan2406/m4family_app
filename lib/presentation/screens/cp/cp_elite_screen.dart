...
/// Web `/cp/elite` parity: Elite Status card + Document Repository + security notice.
class CpEliteScreen extends ConsumerStatefulWidget {
  const CpEliteScreen({super.key});

  @override
  ConsumerState<CpEliteScreen> createState() => _CpEliteScreenState();
}

class _CpEliteScreenState extends ConsumerState<CpEliteScreen> {
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      // CP documents share the user support-documents endpoint when available.
      final api = ref.read(apiClientProvider);
      final res = await api.getMySupportDocuments();
