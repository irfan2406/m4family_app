...
class InvestorPaymentsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<InvestorPaymentsScreen> createState() =>
      _InvestorPaymentsScreenState();
}
...
class _InvestorPaymentsScreenState
    ...
    try {
      ...
      final mapped = schedules.whereType<Map>().map<Map<String, dynamic>>((s) {
        ...
        return {
          'id': (m['_id'] ?? '').toString(),
          'description':
              (m['milestoneName'] ?? projectTitle ?? 'Investment Payment')
                  .toString(),
  ...
  num get _totalInvested => _transactions
      .where(
        (t) =>
            t['direction'] == 'out' &&
            (t['status'] ?? '').toString().toUpperCase() == 'PAID',
      )
      ...
      child: Row(
        children: [
          Expanded(
            child: _statCell(
              'TOTAL INVESTED',
              'AED ${_formatAmount(_totalInvested)}',
              textPrimary,
              muted,
            ),
          ),
          ...
          Expanded(
            child: _statCell(
              'TOTAL RETURNS',
              'AED ${_formatAmount(_totalReturns)}',
              _green,
              muted,
            ),
          ),
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: selected ? _gold.withValues(alpha: 0.4) : border,
              ),
            ),
            child: Text(
              f.toUpperCase(),
  ...
  List<Widget> _buildList(
    ...
    return items
        .map((t) => _buildRow(t, isDark, textPrimary, muted, border))
        .toList();
  }
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(10),
              border: Border.all(
                color: isIn ? _green.withValues(alpha: 0.2) : border,
              ),
            ),
            child: Icon(
