...
void main() {
  test(
    'reads the backend highlights array (exact strings from the live API)',
    () {
      final project = {
        'title': 'Cledor',
        'highlights': ['Prime Location', '20 min from Airport'],
      };
      expect(projectHighlights(project), [
        'Prime Location',
        '20 min from Airport',
      ]);
    },
  ...
  test('falls back to the web defaults when highlights are missing/empty', () {
    ...
    expect(projectHighlights(null), ['Prime Location', '20 min from Airport']);
  });
